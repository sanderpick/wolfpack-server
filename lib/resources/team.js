...
exports.routes = function (app) {
  ...
  app.post('/teams/:n', function (req, res) {
    ...
    Users.read({username: username}, function (err, user) {
      ...
      Teams.create(props, function (err, doc) {
        if (error(err, res)) return;
        res.send(doc);
      });
    });
  ...
  app.get('/teams/:n', function (req, res) {
    ...
    Teams.read({name: req.params.n}, function (err, doc) {
  ...
  app.put('/teams/:n', function (req, res) {
    ...
    Teams.update({name: req.params.n}, {$set: req.body}, function (err, stat) {
  ...
  app.delete('/teams/:n', function (req, res) {
    ...
    Teams.delete({name: req.params.n}, function (err, stat) {
      if (error(err, res, stat, 'team')) return;
  ...
  app.get('/teams/:n/available', function (req, res) {
    ...
    Teams.available({name: req.params.n}, function (err, available) {
      if (error(err, res)) return;
  ...
  app.post('/teams/:n/invite', function (req, res) {
    ...
    Step(
      function () {
        Users.read({username: req.body.inviter}, this.parallel());
        Users.read({username: req.body.invitee}, this.parallel());
      },
      function (err, inviter, invitee) {
        if (error(err, res, inviter, 'inviter')) return;
        if (error(err, res, invitee, 'invitee')) return;

        Teams.read({name: req.params.n}, function (err, team) {
