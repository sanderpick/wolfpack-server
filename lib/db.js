...
exports.Connection = function (uri, options, cb) {
  ...
  this.add = function (name, index, cb) {
    if (!this.db)
    ...
    Step(
      function () {
        self.db.collection(name, _.bind(function (err, col) {
          ...
          if (options.ensureIndexes && index)
            col.ensureIndex(index, this);
          else return;
        }, this));
      },
      function (err) { cb(err); }
...
/*
 * Insert a document adding `created` key if
 * it doesn't exist in the given props.
 */
...
mongodb.Collection.prototype.update = function (query, props, cb) {
  this.update(query, props, {safe: true}, cb);
}
